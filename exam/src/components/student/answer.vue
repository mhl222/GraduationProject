      ...
      <transition name="slider-fade">
        <div class="right">
          <div class="title">
            ...
            <el-button type="primary" plain size="mini" style="height: 80%;margin: 5px 5px ;" v-show="!fullFlag"
              @click="handleFullScreen()">回到全屏</el-button>
            </el-button>
          </div>
          <div class="content">
            ...
            <div v-if="currentType == 1">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                    <el-tag type="success">正确姿势：</el-tag><span class="right">{{reduceAnswer.rightAnswer}}</span>
                  </li>
                  <li>
                    <el-tag>题目解析：</el-tag>
                  </li>
            ...
            <div class="fill" v-if="currentType == 2">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                    <el-tag type="success">正确姿势：</el-tag><span class="right">{{topic[2][index].answer}}</span>
                  </li>
                  <li>
                    <el-tag>题目解析：</el-tag>
                  </li>
            ...
            <div class="judge" v-if="currentType == 3">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                    <el-tag type="success">正确姿势：</el-tag><span class="right">{{topic[3][index].answer}}</span>
                  </li>
                  <li>
                    <el-tag>题目解析：</el-tag>
                  </li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      commit () { //答案提交计算分数
        this.jumpFlag =true
        localStorage.removeItem('saveAnswer');
        /* 计算选择题总分 */
        ...
        topic1Answer.forEach((element, index) => { //循环每道选择题根据选项计算分数
          ...
          if (element != null) {
            ...
            if (right == this.topic[1][index].rightAnswer) { // 当前选项与正确答案对比
              ...
              this.$set(this.topic[1][index], 'show', true)
            } else {
              this.$set(this.topic[1][index], 'show', false)
        ...
        fillAnswer.forEach((element, index) => { //此处index和 this.index数据不一致，注意
          element.forEach((inner) => {
            if (this.topic[2][index].answer.includes(inner)) { //判断填空答案是否与数据库一致
              ...
              this.$set(this.topic[2][index], 'show', true)
            } else {
              this.$set(this.topic[2][index], 'show', false)
            }
          })
        });
        /** 计算判断题总分 */
        ...
        topic3Answer.forEach((element, index) => {
          ...
          if (right == this.topic[3][index].answer) { // 当前选项与正确答案对比
            ...
            this.$set(this.topic[3][index], 'show', true)
          } else {
            this.$set(this.topic[3][index], 'show', false)
          }
        })
      ...
      save () {
        setInterval(() => {
          ...
          localStorage.setItem('saveAnswer', JSON.stringify(obj))
        }, 1000*10)
      },
      getSave (){
        let obj = localStorage.getItem('saveAnswer')
        let {fill,judge,topic} = JSON.parse(obj)
        this.fillAnswer = fill
        this.judgeAnswer = judge
        this.topic1Answer = topic
      },
      ...
      handleFullScreen (dom) {
          ...
          if (this.hasFullScreenElement()) {
            ...
            if (this.timeExit === 0) {
              document.addEventListener('fullscreenchange', () => {
                ...
                if (!isFull&&!this.jumpFlag) {
                  this.timeExit++;
                  setTimeout(()=>{
                    this.$message({
                    showClose: true,
                    message: `已退出${this.timeExit}次！`,
                    type: 'warning'
                  });
                  },1500)
                  ...
                  this.$message({
                    showClose: true,
                    message: `请在10秒内点击按钮返回全屏页面！否则记录违纪行为。`,
                    type: 'error'
                  });
                  this.fullFlag = false;
                }
        ...
        this.$message({
          showClose: true,
          message: `考试结束前请不要退出全屏，退出次数达3次将标识作弊，当前退出次数${this.timeExit}`,
          type: 'success'
        });
      },
      // 判断浏览器是否支持全屏
